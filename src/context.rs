...
pub trait SwaggerService<RequestBody, ResponseBody, Context>:
    Clone
    + Service<Request<ContextualPayload<RequestBody, Context>>,
...
where
    T: Clone
        + hyper::service::Service<Request<ContextualPayload<ReqB, Context>>,
            Response = Response<ResB>,
            Error = hyper::Error,
            Future = 
                Pin<Box<dyn Future<Output = Result<Response<ResB>, hyper::Error>>>>,
        >,
    Context: Has<Option<AuthData>>
        + Has<Option<Authorization>>
        + Has<XSpanIdString>
