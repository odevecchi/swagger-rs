...
impl<Inner, Context, Target> hyper::service::Service<Target> for AddContextMakeService<Inner, Context> 
where
...
impl<Inner, Context, Body> hyper::service::Service<Request<Body>> for AddContextService<Inner, Context>
where
    ...
    Inner: hyper::service::Service<Request<ContextualPayload<Body, Context::Result>>>,
{
   ...
   fn poll_ready(&mut self, context: &mut std::task::Context<'_>) -> Poll<Result<(), Self::Error>> {
